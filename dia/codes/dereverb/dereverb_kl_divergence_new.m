function [y H cost spectrogram_inp spectrogram_out] = dereverb_kl_divergence_new(reverb,sparsity,parm)
...
F = stft_new(reverb, analsize, hp, 0, hwindow);
Fa = abs(F);
Ga = Fa;
...
H = 1-(0:Nframes-1) / (2*Nframes); H = repmat(H, size(Y,1), 1);
...
y = stft_new( eFa .* fphase, analsize, hp, 0, hwindow);
y = y - mean(y);
y = norm_wav(y, 16);
